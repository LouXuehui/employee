...
class Index extends Component {
    ...
    handleAdd() {
        const {dispatch} = this.props
        dispatch({type: 'position/setState', payload: {showEditModal: true}})
    }

    //编辑弹框
    showEditModal(record) {
        const {dispatch} = this.props
        dispatch({type: 'position/getAuthorGroupList', payload: {id: record.ID}}).then(roleList => {
            if (roleList) {
                dispatch({
    ...
    showDeleteModal(record) {
        const {dispatch} = this.props
        dispatch({type: 'position/setState', payload: {userVisible: true, record}})
    }

    //关闭删除提示框
    cancelDeleteModal() {
        const {dispatch} = this.props
        dispatch({type: 'position/setState', payload: {userVisible: false, record: ''}})
    }
    ...
    handleOk() {
        ...
        dispatch({type: 'position/deleteById', payload: {id: record.id}}).then(res => {
            if (res) {
    ...
    clickRow(record, index) {
        const {dispatch} = this.props
        dispatch({type: 'position/setState', payload: {checkedState: record, spinning: true}})
        dispatch({type: 'position/getAuthorGroupList', payload: {id: record.ID}})
    }
...
export default connect(state => Object.assign({}, state.position))(Index)
